...
export class ReportController {
  ...
  async generateReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Validation
      if (!yearRaw || !monthRaw || !state || !district) {
        res.status(400).json({
          success: false,
          error: 'Missing required fields: year, month, state, district',
        });
        return;
      }
      ...
      const existingReport = await prisma.report.findFirst({
        where: {
          ...
          state,
          district,
          // metricCategory is not in the Report model in schema, so we rely on these fields
        },
      });
      ...
      const findings = await this.gatherFindings(year, month, state, district, metricCategory);
      ...
      const filename = `report_${year}_${String(month).padStart(2, '0')}_${state}_${district}_${Date.now()}.pdf`;
      ...
      const report = await prisma.$transaction(async (tx) => {
        ...
        let newReport = await tx.report.create({
          data: {
            title: `Aadhaar Intelligence Report - ${state} ${district}`,
  ...
  private async gatherFindings(
    ...
    const baseWhere = {
      ...
      state,
      district,
      ...(metricCategory && { metricCategory: metricCategory as MetricCategory }),
    };
